...
def watch():
    ...
    if not video_id:
        abort(400) if not_json else jsonify({"error": "Missing v"}), 400

    ip = request.remote_addr
    if not check_ip_limit(ip):
        abort(429) if not_json else jsonify({"error": "Daily limit reached"}), 429
    ...
    if fmt not in ("mp4", "webm"):
        abort(400) if not_json else jsonify({"error": "Invalid format"}), 400

    with LOCK:
        if token in DOWNLOADS:
            abort(409) if not_json else jsonify({"error": "Token in use"}), 409
